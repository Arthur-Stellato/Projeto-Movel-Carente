...
// skip: administradores ficam de fora do limite — quem passa pelo painel admin
// (categorias, denúncias, usuários) naturalmente dispara bem mais chamadas por
// minuto que um usuário comum navegando, e não faz sentido um admin autenticado
// tomar 429 no meio do próprio trabalho. Só é possível decidir isso aqui porque
// `autenticarOpcional` roda antes deste middleware (ver app.js) e já populou
// req.usuario quando o Bearer token é válido — sem isso, o rate limiter nem
// saberia quem está por trás da requisição.
const limitadorGeral = rateLimit({
  ...
  skip: (req) => req.usuario?.tipo === 'admin',
