...
// Mesmo custo de hash usado em src/services/auth.service.js — precisa bater,
// senão o hash gerado aqui não seria compatível com o bcrypt.compare do login.
const SALT_ROUNDS = 10;

// CPF fictício, mas com dígito verificador válido (passa em validarCpf) —
// só existe pra satisfazer a coluna UNIQUE; não corresponde a uma pessoa real.
const CPF_ADMIN_PADRAO = '52998224563';
...
// Cria (ou promove, se já existir por email) um usuário admin — só assim dá
// pra acessar as rotas administrativas (categorias, denúncias, usuários) sem
// precisar mexer direto no banco. Email/senha configuráveis via env pra não
// deixar uma credencial fixa se isso algum dia rodar fora do seu ambiente de
// desenvolvimento/demo.
async function seedAdmin() {
  const email = process.env.SEED_ADMIN_EMAIL || 'admin@movelcarente.com.br';
  ...
  await prisma.usuario.upsert({
    ...
    create: {
      email,
      cpf: CPF_ADMIN_PADRAO,
      senhaHash,
      primeiroNome: 'Admin',
      ultimoNome: 'MóvelCarente',
      tipo: 'admin',
      emailVerificado: true,
      ativo: true,
    },
  ...
  console.log(`Seed concluído: usuário admin pronto (${email} / senha: ${senha}).`);
}

async function main() {
  await seedCentroides();
  await seedAdmin();
}
